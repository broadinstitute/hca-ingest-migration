...
def run_config_for_dcp_release_partition(partition_key:str) -> DagsterObjectConfigSchema:
    path = dg.file_relative_path(
        __file__, os.path.join("./run_config/prod", "dcp_release.yaml")
    )

    # jscpd:ignore-start
    run_config: DagsterObjectConfigSchema = load_yaml_from_path(path)
    run_config["solids"]["pre_process_metadata"]["config"]["input_prefix"] = partition_key
...
def run_config_for_dcp_release_per_project_partition(partition_key:str) -> DagsterObjectConfigSchema:
    path = dg.file_relative_path(
    ...
    project_id = find_project_id_in_str(partition_key)
...
def dev_run_config_for_dcp_release_per_project_partition(partition_key:str) -> DagsterObjectConfigSchema:
    path = dg.file_relative_path(
...
def run_config_per_project_public_snapshot_job(partition_key) -> DagsterObjectConfigSchema:
    path = dg.file_relative_path(
    ...
    project_id, release_tag = partition_key.split(',')
